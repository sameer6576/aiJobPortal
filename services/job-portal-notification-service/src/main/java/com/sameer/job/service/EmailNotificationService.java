...
public class EmailNotificationService {
    ...
    public void sendStatusChangedEmail(ApplicationStatusChangedEvent event) throws Exception {
        try {
            String statusLabel = STATUS_LABELS.getOrDefault(
                    event.getNewStatus(),
                    event.getNewStatus().name()
            );

            String statusColor = STATUS_COLORS.getOrDefault(
                    event.getNewStatus(),
                    "#6b7280"
            );
    ...
    private String buildStatusChangeHtml(
        ...
        return "<!DOCTYPE html>"
                ...
                + "font-size:13px;"
                + "margin-top:24px;'>"
                + "You can log in to the Job Portal to view "
                ...
                + "color:#9ca3af;"
                + "font-size:12px;'>"
                + "Job Portal &mdash; You are receiving this because "
    ...
    private String getStatusMessage(ApplicationStatus status) {
        ...
        return switch (status) {
            ...
            case INTERVIEW_SCHEDULED ->
                    "Great news! An interview has been scheduled "
